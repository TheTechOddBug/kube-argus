...
func computeConfigDrift(pods []*corev1.Pod, cfgMeta, secMeta []configMeta) []interface{} {
...
func apiConfigData(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) != 2 {
		httpx.Error(w, "usage: /api/configs/{namespace}/{name}?kind=configmap|secret", 400)
		return
	}
	...
	httpx.JSONGz(w, r, map[string]interface{}{
		"kind":      kind,
		"name":      name,
		"namespace": ns,
		"entries":   entries,
		"masked":    kind == "secret" && !admin,
	})
}
