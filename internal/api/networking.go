...
func apiIngresses(w http.ResponseWriter, r *http.Request) {
	...
	for _, i := range cache.ingresses.Items {
		...
		if class == "" {
			if v, ok := i.Annotations["kubernetes.io/ingress.class"]; ok {
				class = v
			}
		}
...
func apiIngressDescribe(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) < 2 {
		httpx.Error(w, "use /api/ingresses/{namespace}/{name}", 400)
		return
	}
	...
	for _, s := range cache.services.Items {
		...
		for _, p := range s.Spec.Ports {
			...
			if p.TargetPort.IntValue() > 0 {
				ps += ":" + fmt.Sprintf("%d", p.TargetPort.IntValue())
			}
			if p.NodePort > 0 {
...
func apiServiceDetail(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) < 2 {
		httpx.Error(w, "use /api/services/{namespace}/{name}", 400)
		return
	}
	ns, name := parts[0], parts[1]
