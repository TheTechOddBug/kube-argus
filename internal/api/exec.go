...
func apiExec(w http.ResponseWriter, r *http.Request) {
	...
	if !auth.RequireAdminOrJIT(w, r, ns, ownerKind, ownerName) {
		return
	}
	container := r.URL.Query().Get("container")
	if ns == "" || name == "" {
		httpx.Error(w, "namespace and pod required", 400)
		return
	}
	if sd, ok := r.Context().Value(auth.UserCtxKey).(*auth.SessionData); ok && sd != nil {
		audit.Record(sd.Email, sd.Role, "pod.exec", fmt.Sprintf("Pod %s/%s", ns, name), "container: "+container, auth.ClientIP(r))
	}
