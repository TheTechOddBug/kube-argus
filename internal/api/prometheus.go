...
func apiWorkloadSizing(w http.ResponseWriter, r *http.Request) {
	...
	if nsName == "" || wlName == "" {
		httpx.Error(w, "need namespace and name", 400)
		return
	}
	...
	if podRegex == "" {
		httpx.Error(w, "workload not found", 404)
		return
	}
...
func apiAlerts(w http.ResponseWriter, r *http.Request) {
	if promBaseURL == "" {
		httpx.JSON(w, map[string]interface{}{"alerts": []interface{}{}, "error": "Prometheus not configured"})
		return
	}
	promUser := os.Getenv("PROMETHEUS_USER")
	promKey := os.Getenv("PROMETHEUS_KEY")
	...
	req, err := http.NewRequest("GET", u, nil)
	if err != nil {
		httpx.JSON(w, map[string]interface{}{"alerts": []interface{}{}, "error": err.Error()})
		return
	}
	if promUser != "" && promKey != "" {
		req.SetBasicAuth(promUser, promKey)
	...
	if resp.StatusCode != 200 {
		httpx.JSON(w, map[string]interface{}{"alerts": []interface{}{}, "error": fmt.Sprintf("prometheus %d", resp.StatusCode)})
		return
	}
	...
	httpx.JSON(w, map[string]interface{}{"alerts": alerts})
}
...
func apiMetricsNode(w http.ResponseWriter, r *http.Request) {
	...
	if node == "" {
		httpx.Error(w, `{"error":"node parameter required"}`, 400)
		return
	}
	if rangeStr == "" {
		rangeStr = "1h"
	...
	httpx.JSONGz(w, r, promQueryParallel(queries, startStr, endStr, step))
}
...
func apiMetricsPod(w http.ResponseWriter, r *http.Request) {
	...
	rangeStr := r.URL.Query().Get("range")
	if ns == "" || pod == "" {
		httpx.Error(w, `{"error":"namespace and pod parameters required"}`, 400)
		return
	}
	if rangeStr == "" {
		rangeStr = "1h"
...
func apiMetricsWorkload(w http.ResponseWriter, r *http.Request) {
	...
	rangeStr := r.URL.Query().Get("range")
	if ns == "" || name == "" {
		httpx.Error(w, `{"error":"namespace and name parameters required"}`, 400)
		return
	}
	if kind == "" {
		kind = "deployment"
