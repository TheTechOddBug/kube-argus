...
func apiSpotAdvisor(w http.ResponseWriter, r *http.Request) {
	...
	if spotCache.entries == nil || len(spotCache.entries) == 0 {
		httpx.JSONGz(w, r, map[string]interface{}{"ready": false, "message": "Spot advisor data loading..."})
		return
	}
	...
	if cache.nodes == nil {
		httpx.JSONGz(w, r, map[string]interface{}{"ready": false, "message": "Cluster cache not ready"})
		return
	}
