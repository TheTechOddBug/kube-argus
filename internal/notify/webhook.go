...
// Config is the persisted webhook configuration.
type Config struct {
	URL    string   `json:"url"`
...
// WebhookGetConfig returns a copy of the current webhook configuration.
func WebhookGetConfig() Config {
	webhookCfg.mu.RLock()
...
// InitWebhook loads persisted configuration (ConfigMap takes precedence) and
// falls back to NOTIFY_WEBHOOK_URL / NOTIFY_WEBHOOK_SECRET env vars. Init
...
func webhookRestoreConfig() {
	...
	// One-time migration from the unreleased destinations schema.
	if cfg.URL == "" {
		if destsJSON := cm.Data[webhookCMKeyDestsLegacy]; destsJSON != "" {
			...
			if json.Unmarshal([]byte(destsJSON), &dests) == nil {
				for _, d := range dests {
					if d.Enabled && d.URL != "" {
						cfg = Config{URL: d.URL, Secret: d.Secret, Events: d.Events}
						break
					}
				}
			}
...
// WebhookSettingsHandler handles /api/settings/webhook — GET, PUT, POST(test).
func WebhookSettingsHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "PUT":
		...
		if email, role, ok := CurrentUser(r); ok {
			AuditRecord(email, role, "settings.webhook", "", "updated", ClientIP(r))
		}
	...
	case "POST":
		...
		if secret != "" {
			req.Header.Set(webhookSigHeader, "sha256="+Sign(raw, secret))
		}
		resp, err := http.DefaultClient.Do(req)
		if err != nil {
			httpx.Error(w, fmt.Sprintf("failed to reach webhook: %s", err.Error()), 502)
			return
		}
		defer resp.Body.Close()
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			httpx.Error(w, fmt.Sprintf("webhook returned status %d", resp.StatusCode), 502)
			return
		}
		httpx.JSON(w, map[string]string{"status": "ok"})
...
func ConfigAcceptsEvent(c Config, event string) bool {
	if len(c.Events) == 0 {
		return true
	}
...
func webhookNotifyJIT(event string, req *JITPayload, actor string) {
	cfg := WebhookGetConfig()
	if cfg.URL == "" || req == nil {
		return
	}
...
// Sign computes a hex(HMAC-SHA256(body, secret)) signature for a webhook body.
